...
import { X, Mail, User, PhoneCall, Camera, MapPin, Clock, Building2 } from 'lucide-react-native';
...
export default function PharmacyRegister() {
  ...
  const getCurrentLocation = async () => {
    try {
      ...
      if (status !== 'granted') {
        Alert.alert(
          t('common.error'),
          t('auth.pharmacyRegistration.locationPermissionRequired')
        );
        return;
      }
      ...
      Alert.alert(
        t('common.error'),
        t('auth.pharmacyRegistration.locationError')
      );
    }
  };
  ...
  const validateForm = () => {
    if (!pharmacyName.trim()) {
      Alert.alert(t('common.error'), t('auth.pharmacyRegistration.pharmacyNameRequired'));
      return false;
    }
    if (!email.trim()) {
      Alert.alert(t('common.error'), t('auth.pharmacyRegistration.emailRequired'));
      return false;
    }
    if (!phone.trim()) {
      Alert.alert(t('common.error'), t('auth.pharmacyRegistration.phoneRequired'));
      return false;
    }
    if (!is24Hours && (!openingTime || !closingTime)) {
      Alert.alert(t('common.error'), t('auth.pharmacyRegistration.workingHoursRequired'));
      return false;
    }
    if (!location) {
      Alert.alert(t('common.error'), t('auth.pharmacyRegistration.locationRequired'));
      return false;
    }
    if (!photo) {
      Alert.alert(t('common.error'), t('auth.pharmacyRegistration.photoRequired'));
      return false;
    }
    return true;
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const pharmacyData = {
        ...
        numberOfBranches: numberOfBranches ? parseInt(numberOfBranches) : 1,
        location: location,
        photo: photo,
        type: 'pharmacy'
      };
      ...
      Alert.alert(
        t('common.success'),
        t('auth.pharmacyRegistration.registrationSuccess'),
        [
          {
            text: t('common.ok'),
            onPress: () => router.back()
      ...
      Alert.alert(
        t('common.error'),
        t('auth.pharmacyRegistration.registrationError')
      );
    } finally {
      setIsLoading(false);
    }
      ...
      >
        ...
        <View style={styles.header}>
          ...
          <Text style={styles.headerTitle}>
            {t('auth.pharmacyRegistration.title')}
          </Text>
          <View style={{ width: 24 }} />
        </View>
          ...
          <View style={styles.form}>
            ...
            <View style={styles.inputContainer}>
              ...
              <TextInput
                ...
                placeholder={`${t('auth.pharmacyRegistration.pharmacyName')} *`}
            ...
            <View style={styles.inputContainer}>
              ...
              <TextInput
                style={[styles.input, isRTL && styles.inputRTL]}
                placeholder={`${t('auth.pharmacyRegistration.email')} *`}
                placeholderTextColor="#999"
                value={email}
            ...
            <View style={styles.inputContainer}>
              ...
              <TextInput
                ...
                placeholder={`${t('auth.pharmacyRegistration.phone')} *`}
                placeholderTextColor="#999"
                value={phone}
            ...
            <View style={[styles.inputContainer, styles.textAreaContainer]}>
              <TextInput
                ...
                placeholder={t('auth.pharmacyRegistration.description')}
                placeholderTextColor="#999"
                value={description}
            ...
            <View style={styles.inputContainer}>
              ...
              <TextInput
                style={[styles.input, isRTL && styles.inputRTL]}
                placeholder={t('auth.pharmacyRegistration.numberOfBranches')}
                placeholderTextColor="#999"
            ...
            <View style={styles.workingHoursHeader}>
              <Text style={styles.sectionTitle}>
                {t('auth.pharmacyRegistration.workingHours')} {!is24Hours && <RequiredField />}
              </Text>
              <TouchableOpacity 
              ...
              >
                <Text style={[styles.toggleButtonText, is24Hours && styles.toggleButtonTextActive]}>
                  {t('auth.pharmacyRegistration.24hours')}
                </Text>
              </TouchableOpacity>
            </View>
            ...
            {!is24Hours && (
              <View style={styles.timeContainer}>
                <View style={styles.timeInputContainer}>
                  <Text style={styles.timeLabel}>{t('auth.pharmacyRegistration.openingTime')}</Text>
                  <TouchableOpacity
                ...
                <View style={styles.timeInputContainer}>
                  <Text style={styles.timeLabel}>{t('auth.pharmacyRegistration.closingTime')}</Text>
                  <TouchableOpacity
                    style={styles.timePickerButton}
            ...
            <View style={styles.locationContainer}>
              <View style={styles.locationHeader}>
                ...
                <Text style={styles.locationText}>
                  {location 
                    ? `${t('auth.pharmacyRegistration.locationSet')}: ${location.latitude.toFixed(4)}, ${location.longitude.toFixed(4)}`
                    : `${t('auth.pharmacyRegistration.location')} *`
                  }
                </Text>
              </View>
              <View style={styles.locationButtons}>
                ...
                >
                  <Text style={styles.locationButtonText}>
                    {t('auth.pharmacyRegistration.selectOnMap')}
                  </Text>
                </TouchableOpacity>
                <TouchableOpacity 
                ...
                >
                  <Text style={[styles.locationButtonText, styles.secondaryLocationButtonText]}>
                    {t('auth.pharmacyRegistration.getLocation')}
                  </Text>
                </TouchableOpacity>
              </View>
            </View>
              ...
              {photo ? (
                ...
                <View style={styles.photoPlaceholder}>
                  ...
                  <Text style={styles.photoText}>
                    {t('auth.pharmacyRegistration.uploadPhoto')} <RequiredField />
                  </Text>
                </View>
              )}
            </TouchableOpacity>
            ...
            >
              <Text style={styles.submitButtonText}>
                {isLoading ? t('common.loading') : t('auth.pharmacyRegistration.submit')}
              </Text>
            </TouchableOpacity>
          </View>
        </ScrollView>
          ...
          <View style={styles.timePickerModal}>
            <View style={styles.modalHeader}>
              ...
              <Text style={styles.modalTitle}>
                {showTimePicker.type === 'opening' 
                  ? t('auth.pharmacyRegistration.openingTime')
                  : t('auth.pharmacyRegistration.closingTime')
                }
              </Text>
              <View style={{ width: 24 }} />
            </View>
            ...
            <View style={styles.timePickerContainer}>
              ...
              <View style={styles.timeColumn}>
                <Text style={styles.timeColumnLabel}>{t('auth.pharmacyRegistration.hour')}</Text>
              ...
              {/* Minutes */}
              <View style={styles.timeColumn}>
                <Text style={styles.timeColumnLabel}>{t('auth.pharmacyRegistration.minute')}</Text>
                <ScrollView style={styles.timeScrollView} showsVerticalScrollIndicator={false}>
                  {Array.from({ length: 12 }, (_, i) => i * 5).map((minute) => (
                    <TouchableOpacity
                      ...
                      onPress={() => {
                        const currentTime = showTimePicker.type === 'opening' ? openingTime : closingTime;
                        const currentHour = currentTime ? parseInt(currentTime.split(':')[0]) : 8;
        ...
        <View style={styles.mapModalContainer}>
          <View style={styles.mapModalHeader}>
            ...
            <Text style={styles.mapModalTitle}>
              {t('auth.pharmacyRegistration.selectLocationOnMap')}
            </Text>
            <View style={{ width: 24 }} />
          </View>
          ...
          <View style={styles.mapModalFooter}>
            <Text style={styles.mapModalInstructions}>
              {t('auth.pharmacyRegistration.tapToSelectLocation')}
            </Text>
          </View>
        </View>
      </Modal>
