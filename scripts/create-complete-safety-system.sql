-- نظام الأمان والطوارئ الكامل - إنشاء وإصلاح
-- Complete Safety and Emergency System - Create and Fix
...
CREATE POLICY "Drivers can view passenger medical card during emergency" ON medical_emergency_cards 
    FOR SELECT USING (
        -- المستخدم يرى بطاقته الخاصة
        auth.uid() = user_id
        OR
        -- السائق يرى بطاقة الراكب في الطوارئ (إذا كان جدول trips موجود)
        (
            is_visible = true 
            AND EXISTS (
            ...
            AND EXISTS (
                ...
                WHERE table_name = 'trips' 
                AND column_name IN ('driver_id', 'customer_id')
            )
            AND EXISTS (
        ...
        OR 
        -- الوصول في حالات الطوارئ النشطة
        (
            is_visible = true 
            AND EXISTS (
...
CREATE POLICY "Medical conditions follow card access" ON medical_conditions 
    FOR SELECT USING (
        EXISTS (
...
-- نفس السياسة للأدوية
CREATE POLICY "Medications follow card access" ON current_medications 
    FOR SELECT USING (
        EXISTS (
            SELECT 1 FROM medical_emergency_cards mec 
...
CREATE POLICY "Allergies follow card access" ON medical_allergies 
    FOR SELECT USING (
        EXISTS (
            SELECT 1 FROM medical_emergency_cards mec 
...
CREATE POLICY "Users can insert their own medical conditions" ON medical_conditions 
...
CREATE POLICY "Users can update their own medical conditions" ON medical_conditions 
    FOR UPDATE USING (
        EXISTS (
            SELECT 1 FROM medical_emergency_cards mec 
            WHERE mec.id = medical_conditions.card_id 
...
CREATE POLICY "Users can delete their own medical conditions" ON medical_conditions 
    FOR DELETE USING (
        EXISTS (
            SELECT 1 FROM medical_emergency_cards mec 
            WHERE mec.id = medical_conditions.card_id 
...
CREATE POLICY "Users can insert their own medications" ON current_medications 
    FOR INSERT WITH CHECK (
        EXISTS (
            SELECT 1 FROM medical_emergency_cards mec 
            WHERE mec.id = current_medications.card_id 
...
CREATE POLICY "Users can update their own medications" ON current_medications 
    FOR UPDATE USING (
        EXISTS (
            SELECT 1 FROM medical_emergency_cards mec 
            WHERE mec.id = current_medications.card_id 
...
CREATE POLICY "Users can delete their own medications" ON current_medications 
    FOR DELETE USING (
        EXISTS (
            SELECT 1 FROM medical_emergency_cards mec 
            WHERE mec.id = current_medications.card_id 
...
CREATE POLICY "Users can insert their own allergies" ON medical_allergies 
    FOR INSERT WITH CHECK (
        EXISTS (
            SELECT 1 FROM medical_emergency_cards mec 
            WHERE mec.id = medical_allergies.card_id 
...
CREATE POLICY "Users can update their own allergies" ON medical_allergies 
    FOR UPDATE USING (
        EXISTS (
            SELECT 1 FROM medical_emergency_cards mec 
            WHERE mec.id = medical_allergies.card_id 
...
CREATE POLICY "Users can delete their own allergies" ON medical_allergies 
    FOR DELETE USING (
        EXISTS (
            SELECT 1 FROM medical_emergency_cards mec 
            WHERE mec.id = medical_allergies.card_id 
...
CREATE POLICY "Users can view their own incidents" ON emergency_incidents 
    FOR SELECT USING (auth.uid() = user_id);

CREATE POLICY "Users can insert their own incidents" ON emergency_incidents 
    FOR INSERT WITH CHECK (auth.uid() = user_id);

CREATE POLICY "Users can update their own incidents" ON emergency_incidents 
    FOR UPDATE USING (auth.uid() = user_id);

-- سياسات إعدادات الأمان
CREATE POLICY "Users can view their own safety settings" ON safety_settings 
    FOR SELECT USING (auth.uid() = user_id);

CREATE POLICY "Users can insert their own safety settings" ON safety_settings 
    FOR INSERT WITH CHECK (auth.uid() = user_id);

CREATE POLICY "Users can update their own safety settings" ON safety_settings 
    FOR UPDATE USING (auth.uid() = user_id);
...
CREATE POLICY "Users can insert their own trip shares" ON trip_shares 
    FOR INSERT WITH CHECK (auth.uid() = user_id);

CREATE POLICY "Users can update their own trip shares" ON trip_shares 
    FOR UPDATE USING (auth.uid() = user_id);
