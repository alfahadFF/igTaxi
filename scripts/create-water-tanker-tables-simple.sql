-- نسخة مبسطة من جداول صهاريج المياه بدون PostGIS
-- يمكن استخدامها إذا لم يكن PostGIS متاحاً
...
-- إنشاء فهارس للبحث السريع (بدون PostGIS)
CREATE INDEX idx_water_tankers_location ON water_tankers (current_latitude, current_longitude);
...
-- دالة لحساب المسافة باستخدام معادلة Haversine (بدون PostGIS)
...
BEGIN
    ...
    RETURN ROUND((r * c)::DECIMAL(8, 2), 2);
END;
$$ LANGUAGE plpgsql;
...
-- دالة للبحث عن أقرب صهريج متاح (بدون PostGIS)
...
BEGIN
    ...
    SELECT 
        ...
        wt.rating,
        wt.total_trips,
        calculate_distance_km(wt.current_latitude, wt.current_longitude, delivery_lat, delivery_lng) as distance_km,
        wt.current_latitude,
    ...
    WHERE 
        ...
        AND calculate_distance_km(wt.current_latitude, wt.current_longitude, delivery_lat, delivery_lng) <= max_distance_km
    ORDER BY 
        calculate_distance_km(wt.current_latitude, wt.current_longitude, delivery_lat, delivery_lng) ASC,
        wt.rating DESC
    LIMIT limit_count;
END;
$$ LANGUAGE plpgsql;
...
-- إنشاء ملفات تعريف تجريبية للسائقين (إذا لم تكن موجودة)
INSERT INTO profiles (id, full_name, phone, user_type, created_at)
VALUES 
(
    '00000000-0000-0000-0000-000000000001',
    'أحمد محمد - سائق صهريج',
    '+962791234567',
    'driver',
    NOW()
),
...
ON CONFLICT (id) DO NOTHING; -- تجاهل إذا كان الملف موجوداً

-- إدراج بيانات تجريبية للصهاريج
...
ON CONFLICT (license_plate) DO NOTHING; -- تجاهل إذا كان الصهريج موجوداً
